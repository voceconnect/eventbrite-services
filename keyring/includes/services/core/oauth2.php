...
class Keyring_Service_OAuth2 extends Keyring_Service_OAuth1 {
	...
	function request_token() {
		...
		if ( !isset( $_REQUEST['nonce'] ) || !wp_verify_nonce( $_REQUEST['nonce'], 'keyring-request-' . $this->get_name() ) ) {
			Keyring::error( __( 'Invalid/missing request nonce.', 'eventbrite-parent' ) );
			exit;
		}
	...
	function verify_token() {
		...
		if ( !isset( $_GET['code'] ) || !isset( $_GET['state']) ) {
			Keyring::error(
				sprintf( __( 'There was a problem authorizing with %s. Please try again in a moment.', 'eventbrite-parent' ), $this->get_label() )
			);
			return false;
		}
		...
		if ( !$keyring_request_token ) {
			Keyring::error(
				sprintf( __( 'Failed to load your request token while connecting to %s. Please try again in a moment.', 'eventbrite-parent' ), $this->get_label() )
			);
			return false;
		}
	...
	function request( $url, array $params = array() ) {
		...
		default:
			Keyring::error( __( 'Unsupported method specified for request_token.', 'eventbrite-parent' ) );
			exit;
		}
