...
class Keyring_Service_HTTP_Basic extends Keyring_Service {
	...
	function request_ui() {
		...
		echo '<h2>' . __( 'Account Details', 'eventbrite-parent' ) . '</h2>';
		...
		if ( isset( $_GET['error'] ) ) {
			...
			switch ( $_GET['error'] ) {
			case '401':
				echo '<li>' . __( 'Your account details could not be confirmed, please try again.', 'eventbrite-parent' ) . '</li>';
				break;
			case 'empty':
				echo '<li>' . __( 'Please make sure you enter a username and password.', 'eventbrite-parent' ) . '</li>';
				break;
			}
			echo '</ul></div>';
		}
		...
		echo '<p>' . sprintf( __( 'Enter your username and password for accessing <strong>%s</strong>:', 'eventbrite-parent' ), $this->get_label() ) . '</p>';
		...
		echo '<tr><th scope="row">' . __( 'Username', 'eventbrite-parent' ) . '</th>';
		echo '<td><input type="text" name="username" value="" id="username" class="regular-text"></td></tr>';
		echo '<tr><th scope="row">' . __( 'Password', 'eventbrite-parent' ) . '</th>';
		echo '<td><input type="password" name="password" value="" id="password" class="regular-text"></td></tr>';
		echo '</table>';
		echo '<p class="submitbox">';
		echo '<input type="submit" name="submit" value="' . __( 'Verify Details', 'eventbrite-parent' ) . '" id="submit" class="button-primary">';
		echo '<a href="' . esc_url( $_SERVER['HTTP_REFERER'] ) . '" class="submitdelete" style="margin-left:2em;">' . __( 'Cancel', 'eventbrite-parent' ) . '</a>';
		echo '</p>';
		echo '</form>';
		echo '</div>';
	...
	function verify_token() {
		if ( !isset( $_REQUEST['nonce'] ) || !wp_verify_nonce( $_REQUEST['nonce'], 'keyring-verify-' . $this->get_name() ) ) {
			Keyring::error( __( 'Invalid/missing verification nonce.', 'eventbrite-parent' ) );
			exit;
		}
	...
	function request( $url, array $params = array() ) {
		if ( $this->requires_token() && empty( $this->token ) )
			return new Keyring_Error( 'keyring-request-error', __( 'No token', 'eventbrite-parent' ) );
		...
		default:
			Keyring::error( __( 'Unsupported method specified for verify_token.', 'eventbrite-parent' ) );
			exit;
		}
