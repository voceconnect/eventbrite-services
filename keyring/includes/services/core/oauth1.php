...
class Keyring_Service_OAuth1 extends Keyring_Service {
	...
	function request_token() {
		...
		if ( 200 == wp_remote_retrieve_response_code( $res ) ) {
			...
			Keyring::error(
				sprintf( __( 'There was a problem connecting to %s to create an authorized connection. Please try again in a moment.', 'eventbrite-parent' ), $this->get_label() )
			);
			return false;
		}
	...
	function request( $url, array $params = array() ) {
		...
		default:
			Keyring::error( __( 'Unsupported method specified.', 'eventbrite-parent' ) );
			exit;
		}
