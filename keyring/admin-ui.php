...
class Keyring_Admin_UI {
	...
	function admin_menu() {
		$hook = add_management_page( __( 'Keyring', 'eventbrite-parent' ) , __( 'Keyring', 'eventbrite-parent' ), 'read', 'keyring', array( $this, 'admin_page' ), '' );
	...
	function admin_page_header( $screen = false ) {
		...
		switch ( $screen ) {
		case 'tokens' :
			echo '<h2>' . __( 'Keyring: Managed Connections', 'eventbrite-parent' ) . ' <a href="' . esc_url( Keyring_Util::admin_url( false, array( 'action' => 'services' ) ) ) . '" class="add-new-h2">' . __( 'Add New', 'eventbrite-parent' ) . '</a></h2>';
			break;
		case 'services' :
			echo '<h2>' . __( 'Add New Connection', 'eventbrite-parent' ) . '</h2>';
			echo '<p><a href="' . esc_url(  Keyring_Util::admin_url() ) . '">' . __( '&larr; Back', 'eventbrite-parent' ) . '</a></p>';
			break;
		case 'error' :
			echo '<h2>' . __( 'Keyring Error!', 'eventbrite-parent' ) . '</h2>';
			break;
		default :
			echo '<h2>' . __( 'Keyring', 'eventbrite-parent' ) . '</h2>';
		}
		...
		if ( $this->keyring->has_errors() ) {
			...
			echo '</ul></div>';
			echo '<p class="submit"><a href="' . esc_url( Keyring_Util::admin_url( $_REQUEST['service'] ) ) . '" class="button-primary">' . __( 'Start Again', 'eventbrite-parent' ) . '</a></p>';
			return;
		}
	...
	function admin_page() {
		...
		if ( isset( $_REQUEST['action'] ) && 'delete' == $_REQUEST['action'] ) {
			if ( !isset( $_REQUEST['nonce'] ) || !wp_verify_nonce( $_REQUEST['nonce'], 'keyring-delete-' . $_REQUEST['service'] . '-' . $_REQUEST['token'] ) ) {
				Keyring::error( __( 'Invalid/missing delete nonce.', 'eventbrite-parent' ) );
				exit;
			}

			if ( $this->keyring->get_token_store()->delete( array( 'id' => (int) $_REQUEST['token'], 'type' => 'access' ) ) )
				Keyring::message( __( 'That connection has been deleted.', 'eventbrite-parent' ) );
			else
				Keyring::message( __( 'Could not delete that connection!', 'eventbrite-parent' ) );
		}
		...
		case 'services' :
			...
			if ( count( $services ) ) {
				...
				if ( count( $configured ) ) {
					echo '<p><strong>' . __( 'Click a service to create a new connection:', 'eventbrite-parent' ) . '</strong></p>';
					echo '<ul>';
					...
					echo '<p>' . __( 'There are no fully-configured services available to connect to.', 'eventbrite-parent' ) . '</p>';
				}

				if ( count( $not_configured ) ) {
					echo '<p>' . __( 'The following services need to be configured correctly before you can connect to them.', 'eventbrite-parent' ) . '</p>';
					echo '<ul>';
...
class Keyring_Connections_List_Table extends WP_List_Table {
	...
	function no_items() {
		echo '<p>' . sprintf( __( "You haven't added any connections yet. <a href='%s'>Add a New Connection</a>.", 'eventbrite-parent' ), esc_url( Keyring_Util::admin_url( false, array( 'action' => 'services' ) ) ) ) . '</p>';
	}

	function get_columns() {
		return array(
			'service'  => __( 'Service', 'eventbrite-parent' ),
			'avatar'   => __( 'Avatar', 'eventbrite-parent' ),
			'id'       => __( 'External ID', 'eventbrite-parent' ),
			'name'     => __( 'Name', 'eventbrite-parent' ),
			'actions'  => '&nbsp;'
		);
	}

	function column_service( $row ) {
		echo esc_html( $row->get_service()->get_label() );
	}

	function column_avatar( $row ) {
		$picture = $row->get_meta( 'picture' );
		if ( $picture ) {
			echo '<img src="' . esc_url( $picture ) . '" width="40" height="40" border="1" alt="' . __( 'Avatar', 'eventbrite-parent' ) . '" />';
		} else {
			echo '-';
		}
	}
	...
	function column_id( $row ) {
		echo intval( $row->get_meta( 'user_id' ) );
	}
	...
	function column_actions( $row ) {
		...
		echo '<a href="' . esc_url(	Keyring_Util::admin_url( false, array( 'action' => 'delete', 'service' => $row->get_service()->get_name(), 'token' => $row->get_uniq_id(), 'kr_nonce' => $kr_nonce, 'nonce' => $delete_nonce ) ) ) . '" title="' . esc_attr( __( 'Delete', 'eventbrite-parent' ) ) . '" class="delete">' . __( 'Delete', 'eventbrite-parent' ) . '</a>';
	}
}
