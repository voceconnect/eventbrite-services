...
abstract class Keyring_Service {
	...
	function basic_ui() {
		if ( !isset( $_REQUEST['nonce'] ) || !wp_verify_nonce( $_REQUEST['nonce'], 'keyring-manage-' . $this->get_name() ) ) {
			Keyring::error( __( 'Invalid/missing management nonce.', 'eventbrite-parent' ) );
			exit;
		}
		...
		echo '<h2>' . __( 'Keyring Service Management', 'eventbrite-parent' ) . '</h2>';
		echo '<p><a href="' . esc_url( Keyring_Util::admin_url( false, array( 'action' => 'services' ) ) ) . '">' . __( '&larr; Back', 'eventbrite-parent' ) . '</a></p>';
		echo '<h3>' . sprintf( __( '%s API Credentials', 'eventbrite-parent' ), esc_html( $this->get_label() ) ) . '</h3>';
		...
		if ( isset( $_POST['api_key'] ) && isset( $_POST['api_secret'] ) ) {
			...
			echo '<div class="updated"><p>' . __( 'Credentials saved.', 'eventbrite-parent' ) . '</p></div>';
		}
		...
		echo '<input type="submit" name="submit" value="' . __( 'Save Changes', 'eventbrite-parent' ) . '" id="submit" class="button-primary">';
