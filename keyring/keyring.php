...
class Keyring {
	...
	static function plugins_loaded() {
		...
		if ( !defined( 'KEYRING__TOKEN_STORE' ) || !class_exists( KEYRING__TOKEN_STORE ) || !in_array( 'Keyring_Store', class_parents( KEYRING__TOKEN_STORE ) ) )
			wp_die( sprintf( __( 'Invalid <code>KEYRING__TOKEN_STORE</code> specified. Please make sure <code>KEYRING__TOKEN_STORE</code> is set to a valid classname for handling token storage in <code>%s</code> (or <code>wp-config.php</code>)', 'eventbrite-parent' ), __FILE__ ) );
	...
	static function request_handlers() {
		...
		if (
			...
			if ( !isset( $_REQUEST['kr_nonce'] ) || !wp_verify_nonce( $_REQUEST['kr_nonce'], 'keyring-' . $_REQUEST['action'] ) ) {
				Keyring::error( __( 'Invalid/missing Keyring core nonce. All core actions require a valid nonce.', 'eventbrite-parent' ) );
				exit;
			}
	...
	static function error( $str, $info = array() ) {
		...
		wp_die( $str, __( 'Keyring Error', 'eventbrite-parent' ) );
		exit;
	}
...
class Keyring_Util {
	...
	static function token_select_box( $tokens, $name, $create = false ) {
		...
		<?php if ( $create ) : ?>
			<option value="new"><?php _e( 'Create a new connection&hellip;', 'eventbrite-parent' ); ?></option>
		<?php endif; ?>
		<?php foreach ( (array) $tokens as $token ) : ?>
			<option value="<?php echo esc_attr( $token->get_uniq_id() ); ?>"><?php echo esc_attr( $token->get_display() ); ?></option>
		<?php endforeach; ?>
		</select><?php
	}
